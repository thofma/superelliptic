...
#
    ...
    if n == 1
        if fits(Int64, FlintZZ(p)^(N+1))
            ...
            return _AbsoluteFrobeniusAction_and_the_other_steps(R0, R1, a, hbar, N)
        else
...
function _AbsoluteFrobeniusAction_and_the_other_steps(R0, R1, a, hbar, N)
    K = base_ring(hbar)
    p = convert(Int64,characteristic(K))
    q = order(K)
    n = degree(K)
...
#
    ...
    Chi = AbsoluteFrobeniusAction(a, hbar, N)
